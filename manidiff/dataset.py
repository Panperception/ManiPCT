...
class ShapeNet15kPointClouds(Uniform15KPC):
    def __init__(self, root_dir="D:\\diffusion\\datasets\\shapenetv2.pc15k\\ShapeNetCore.v2.PC15k",
                 categories=['airplane'], tr_sample_size=10000, te_sample_size=5000,
        ...
        if 'all' in categories:
            self.synset_ids = [os.path.join(root_dir, s) for s in cate_to_synsetid.values()]
        else:
            self.synset_ids = [os.path.join(root_dir, cate_to_synsetid[c]) for c in categories]
...
args = get_args()
shape_ds = ShapeNet15kPointClouds(categories=['chair'], split='train')
loaders = get_data_loaders(args)
train_loader = loaders['train_loader']
# if __name__ == '__main__':
#     for point in train_loader:
#         xyz = point['train_points'][0].detach().numpy()
#
